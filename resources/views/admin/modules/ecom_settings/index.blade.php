                    ...
                    <table id="datatable-buttons" class="table table-striped dt-responsive js-exportable nowrap w-100">
                        ...
                        <form class="d-inline" action="{{route('ecom-settings.update',$ecomSetting->id)}}" method="post" data-parsley-validate>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="currency_id" id="" class="form-control" required="required" >
                                        ...
                                        @foreach($currencies as $currency)
                                            <option value="{{$currency->id}}" {{$currency->id==$ecomSetting->currency_id ? 'selected' : ''}}>{{$currency->currency_name}}</option>
                                        @endforeach
                                    </select>
                                    @error('currency_id') <span class="text-danger">{{$message}}</span> @enderror
                                </td>
                                <td>
                                    <input class="form-control" type="number" name="shipping_cost" required="required" value="{{$ecomSetting->shipping_cost  ?? old('shipping_cost') }}">
                                    @error('shipping_cost') <span class="text-danger">{{$message}}</span> @enderror
                                </td>
                                </td>
                                <td>
                                    <input class="form-control" type="number" name="outer_city_shipping_cost" required="required" value="{{$ecomSetting->outer_city_shipping_cost  ?? old('outer_city_shipping_cost') }}">
                                    @error('outer_city_shipping_cost') <span class="text-danger">{{$message}}</span> @enderror
                                </td>
                                <td>
                                    <input class="form-control" type="text" name="admin_email" required="required" value="{{$ecomSetting->admin_email  ?? old('admin_email') }}">
                                ...
                                <button class="btn btn-outline-success table-btn btn-sm" title="Update"><i class="fa fa-check"></i></button>
                                </td>
                            </tr>
                        </form>
                        </tbody>
